...
/*余额额支付密码_更改_START_bbs.hongyuvip.com*/
'act_forget_pass', 're_pass', 'auction_list', 'my_recommend', 'forget_surplus_password', 'act_forget_surplus_password', 'update_surplus_password', 'act_update_surplus_password', 'verify_reset_surplus_email', 'get_verify_code'
); // 代码修改
   // By
   // bbs.hongyuvip.com
/* 余额额支付密码_更改_END_bbs.hongyuvip.com */
...
function action_voucher_list()
{
	...
	//该用户的所有订单
	$sql = "select vc_name,vc_starttime,vc_endtime from ecs_voucher left join ecs_voucher_order on ecs_voucher.vc_id=ecs_voucher_order.vco_voucher_id 
	where ecs_voucher_order.vco_user_id=$user_id";
	$sql1 = "select vco_activation_status from ecs_voucher_order left join ecs_voucher on ecs_voucher.vc_id=ecs_voucher_order.vco_voucher_id 
	where ecs_voucher_order.vco_user_id=$user_id";
	//$sql = "SELECT u.user_money,v.vc_name,v.vc_saleprice FROM ".$GLOBALS['ecs']->table('voucher_order')." as vo left join ".$GLOBALS['ecs']->table('voucher')." as v on vo.vco_voucher_id=v.vc_id left join ".$GLOBALS['ecs']->table('users')." as u on u.user_id = vo.vco_user_id WHERE vo.vco_id = ".$vcoid;
	$voucher = $db->getAll($sql);
	$voucher1 = $db->getAll($sql1);
...
{   
	...
	$smarty = $GLOBALS['smarty'];
	$user_id = $_SESSION['user_id'];
	$recommend_user_list = $db->getAll("SELECT * FROM " . $ecs->table("users") . " WHERE parent_id = '$user_id' ORDER BY reg_time DESC");
	foreach($recommend_user_list as $key=>$val){
		$recommend_user_list[$key]['reg_time'] = date("Y-m-d H:i:s",$val['reg_time']);
	}
	$my_url = "http://www.furongph.com/register.php?u={$user_id}";
	$smarty->assign('recommend_user_list',$recommend_user_list);
	$qr_url = "http://pan.baidu.com/share/qrcode?w=300&h=300&url={$my_url}";
	$smarty->assign('qr_url',$qr_url);
	$smarty->assign('my_url',$my_url);
	$smarty->display('user_clips.dwt');
