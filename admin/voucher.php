...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('voucher_type.htm');
}
...
if ($_REQUEST['act'] == 'remove')
{
	...
	// $sql="select count(*) from ". $ecs->table('voucher') ." where vc_id='$vc_id' ";
...
{  
    ...
    /* 检查类型是否有重复 */
    // $sql = "SELECT COUNT(*) FROM " .$ecs->table('voucher_type'). " WHERE type_name='$type_name'";
    // if ($db->getOne($sql) > 0)
    // {
...
elseif ( $_REQUEST['act'] == 'add_goods' )
{
			$smarty->assign('full_page',    1);
			 ...
			 $sql="select goods_ids from ". $ecs->table('voucher_type_goods') ." where tg_type_id='$type_id' ";
			 ...
			 assign_query_info();
			 $smarty->display('voucher_addgoods.htm');
}
...
elseif ( $_REQUEST['act'] == 'add_goods_update' )
{
		...
		{
			$link[] = array('text' => '返回上一页', 'href' => 'voucher.php?act=tg_list&tg_type='.$type_id );					
		}
		else
		{
			$link[] = array('text' =>  '返回上一页', 'href' => 'voucher.php?act=list');
		}

		
		$sql = "select type_money from ". $ecs->table('voucher_type') ." where type_id='$type_id' ";
		$type_money = $db->getOne($sql);
		if ($goods_ids)
		{
			//配置商品价格 跟  提货券价格做判断
			if ($_CFG['voucher_check_money']) 
			{
				$sql= "select goods_id, goods_name, shop_price from ". $ecs->table('goods') ." where goods_id ".db_create_in($goods_ids);
				$res = $db->query($sql);
				while($row=$db->fetchRow($res))
		...
		if ($_POST['add_type'] == 'tglist')
		{
			...
			if ($tg_ids)
			{
				...
				{
					$sql="replace into ".$ecs->table('voucher_goods')."(tg_id, goods_ids) values('$tg_id', '$goods_ids')";
					$db->query($sql);
				}
			}
		}
		...
		{
			//针对类型进行配置
			$sql="replace into ".$ecs->table('voucher_type_goods')."(tg_type_id, goods_ids) values('$type_id', '$goods_ids')";
			$db->query($sql);
		}
...
if ($_REQUEST['act'] == 'send')
{
    ...
    $smarty->assign('ur_here',      $_LANG['send_voucher']);
    $smarty->assign('action_link',  array('href' => 'voucher.php?act=list', 'text' => $_LANG['voucher_type_list']));

    $smarty->assign('type_id',    $id);

    $smarty->display('voucher_send.htm');
...
if ($_REQUEST['act'] == 'send_by_print')
{
	...
	$sql="select tg_sn from ".$ecs->table('voucher')." where type_id='$type_id' order by tg_sn desc limit 0,1";
    ...
    {		
		...
		$db->query("INSERT INTO ".$ecs->table('voucher')." (type_id, tg_sn, tg_pwd, add_time) VALUES('$type_id', '$tg_sn', '$tg_pwd', '$add_time')");
		$j++;
		$tg_sn_num++;
    }
    ...
    /* 提示信息 */
    $link[0]['text'] = $_LANG['back_voucher_list'];
    $link[0]['href'] = 'voucher.php?act=tg_list&tg_type=' . $type_id.'&is_used=-1';

    sys_msg($_LANG['creat_voucher'] . $j . $_LANG['creat_voucher_num'], 0, $link);
}
...
if ($_REQUEST['act'] == 'gen_excel')
{
    ...
    $type_name = $db->getOne("SELECT type_name FROM ".$ecs->table('voucher_type')." WHERE type_id = '$type_id'");

    /* 文件名称 */
    $voucher_filename = $type_name .'_voucher_list';
    if (EC_CHARSET != 'gbk')
    {
        $voucher_filename = ecs_iconv('UTF8', 'GB2312',$voucher_filename);
    }

    header("Content-type: application/vnd.ms-excel; charset=utf-8");
    header("Content-Disposition: attachment; filename=$voucher_filename.xls");
    ...
    $sql = "SELECT tg.tg_id, tg.type_id, tg.tg_sn, tg.tg_pwd,  tt.type_name, tt.type_money, tt.type_money_count, tt.use_start_date, tt.use_end_date ".
           "FROM ".$ecs->table('voucher')." AS tg, ".$ecs->table('voucher_type')." AS tt ".
...
// {
// 	$smarty->assign('full_page',    1);
// 	$smarty->assign('ur_here',      $_LANG['voucher_order_info']);

// 	if (!empty($_GET['tgid']))
// 	{
// 		$smarty->assign('action_link',   array('href' => 'voucher.php?act=order_list&tg_type='.$_GET['tg_type'].'&is_used='.$_GET['is_used'].'&tgid='.$_GET['tgid'], 'text' => $_LANG['voucher_order_list']));
// 	}
// 	else
// 	{
// 		$smarty->assign('action_link',   array('href' => 'voucher.php?act=order_list', 'text' => $_LANG['voucher_order_list']));
// 	}

// 	$id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
// 	$sql = "select * from ". $ecs->table('voucher_order') ."  where rec_id='$id' ";
// 	$order = $db->getRow($sql);   
// 	if ($order)
// 	{
...
// 	$sql = "SELECT MAX(rec_id) FROM " . $ecs->table('voucher_order') . " WHERE rec_id < '$order[rec_id]'";
// 	$smarty->assign('prev_id', $db->getOne($sql));
// 	$sql = "SELECT MIN(rec_id) FROM " . $ecs->table('voucher_order') . " WHERE rec_id > '$order[rec_id]'";
// 	$smarty->assign('next_id', $db->getOne($sql));

// 	assign_query_info();
// 	$smarty->display('voucher_order_view.htm');
// }
...
{
    $smarty->assign('full_page',    1);
    $smarty->assign('ur_here',      $_LANG['voucher_list']);
    $smarty->assign('action_link',   array('href' => 'voucher.php?act=list', 'text' => $_LANG['voucher_type_list']));
	$smarty->assign('action_link2',   array('href' => 'voucher.php?act=gen_excel&tg_type='.$_REQUEST['tg_type'], 'text' => $_LANG['gen_excel']));
    ...
    assign_query_info();
    $smarty->display('voucher_list.htm');
}
...
if ($_REQUEST['act'] == 'query_bonus')
{
    ...
    make_json_result($smarty->fetch('voucher_list.htm'), '',
        array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
...
{
    check_authz_json('voucher_list');

    $id = intval($_GET['id']);

    $db->query("DELETE FROM " .$ecs->table('voucher'). " WHERE tg_id='$id'");

    $url = 'voucher.php?act=query_bonus&' . str_replace('act=remove_bonus', '', $_SERVER['QUERY_STRING']);
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        if (isset($_POST['drop']))
        {
            $sql = "DELETE FROM " . $ecs->table('voucher'). " WHERE tg_id " . db_create_in($tg_id_list);
            $db->query($sql);

            clear_cache_files();

            $link[] = array('text' => $_LANG['back_voucher_list'],
                'href' => 'voucher.php?act=tg_list&tg_type='. $type_id.'&is_used=-1');
		...
		{   
		     ...
		     //ecs_header("Location:voucher.php?act=add_goods\n");
		}
...
function get_type_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('voucher');
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $row['send_count'] = isset($sent_arr[$row['vc_id']]) ? $sent_arr[$row['vc_id']] : 0;
 ...
 * @return void
 */
// function get_voucher_order($tgid)
// {
...
//     $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('voucher_order'). ' AS tgo '.$where;
//     $filter['record_count'] = $GLOBALS['db']->getOne($sql);
...
//           " FROM " .$GLOBALS['ecs']->table('voucher_order'). " AS tgo ".
...
//  //    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('voucher'). ' AS tg '.$where;
//  //    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
//     // $filter['sprice']     = empty($_REQUEST['sprice']) ? '0' : trim($_REQUEST['sprice']);
//     // $filter['bprice']     = empty($_REQUEST['bprice']) ? '0' : trim($_REQUEST['bprice']);
    
//     //$sql = "SELECT * FROM ".$GLOBALS['ecs']->table('voucher'). ' AS tg '. WHERE price BETWEEN .$filter['sprice']. AND .$filter['bprice'];
...
//           " FROM ".$GLOBALS['ecs']->table('voucher'). " AS tg ".
//           " LEFT JOIN " .$GLOBALS['ecs']->table('voucher_order'). " AS tgo ON tgo.rec_id = tg.tg_order_id   ".
// 		" LEFT JOIN ". $GLOBALS['ecs']->table('voucher_goods') ." AS tgg on tg.tg_id =tgg.tg_id  ".
//           " $where ORDER BY ".$filter['sort_by']." ".$filter['sort_order'].
//           " LIMIT ". $filter['start'] .", $filter[page_size]";
//     $row = $GLOBALS['db']->getAll($sql);
...
// 		$row[$key]['is_used'] =  $val['used_time'] ? "<font color=#ff3300>".$num_used." &nbsp; [ <a href='voucher.php?act=order_list&tg_type=".$_GET['tg_type']."&is_used=".$_GET['is_used']."&tgid=".$val['tg_id']."'>查看</a> ]</font>" : "未使用";
//     }
...
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('voucher_type') .
...
function get_order_comment_list()
{
    ...
    if ($filter['keyword'])
    {
        ...
        $where.=" AND vc_price = ". $filter['keyword'];
    }
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('voucher'). $where . " ORDER BY vc_id DESC";
